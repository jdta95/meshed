...
where $w\sim MGP$ has unitary variance. This model is equivalent to the previous one. 

```{r}
summary(meshout$theta_mcmc["sigmasq",,])
summary(meshout$theta_mcmc["phi",,])
...
```{r}
# recover latent effects and calculate means
wmesh <- recover_W(meshout) %>% apply(1, mean)

# calculate yhat means
ymesh <- meshout$yhat_mcmc %>% apply(1, mean)

meshout$coordsdata %>%
  ggplot(aes(Var1, Var2, fill = wmesh)) + 
    geom_raster() +
    scale_fill_viridis_c() +
    theme_minimal() + ggtitle("w: recovered")

meshout$coordsdata %>%
  ggplot(aes(Var1, Var2, fill = ymesh)) +
    geom_raster() +
    scale_fill_viridis_c() +

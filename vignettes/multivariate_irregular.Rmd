...
y_post_sample <- meshout$yhat_mcmc %>% apply(1:2, mcmc_summary)

# posterior mean for 3rd outcome:
class(meshout$coordsdata)
head(meshout$coordsdata)
head(coords)

meshout$coordsdata %>%
  ggplot(aes(Var1, Var2, color = y_post_sample[2,,3])) +
    geom_point() +
...
```{r}
# missing values for 3rd outcome
pred_ix = is.na(YY[, 3])

# correlation between predictions and true values for unobserved locations
cor(y_post_sample[2, pred_ix, 3], YY_full[pred_ix, 3])
```
